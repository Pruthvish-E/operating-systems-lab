...
int main(int argc, char **argv)
{
    ...
    // copy the string LIST passed as argv[1] to recvBuff
    // write() system call to send string stored in recvBuff using "sockfd" file descriptor
    write(sockfd,argv[1], sizeof(recvBuff));
    ...
    write(sockfd, argv[2], sizeof(recvBuff));
    ...
    int selected_file_fd = open("test.txt", O_CREAT | O_RDWR);
        ...
        int b = 0;
        while((b = read(sockfd, recvBuff, sizeof(recvBuff))) > 0){
          printf("%s", recvBuff);
          write(selected_file_fd, recvBuff, sizeof(recvBuff));
        }
